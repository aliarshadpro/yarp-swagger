...
public sealed class ReverseProxyDocumentFilter : IDocumentFilter
{
    ...
    )
    {
        ...
        foreach ((string clusterKey, ReverseProxyDocumentFilterConfig.Cluster cluster) in clusters)
        {
            if (true != cluster.Destinations?.Any()) continue;

            foreach (var destination in cluster.Destinations)
            {
                if (true != destination.Value.Swaggers?.Any()) continue;
                ...
                {
                    if (swagger.Paths?.Any() != true) continue;

                    IReadOnlyDictionary<string, IEnumerable<string>> publishedRoutes = null;
                    if (swagger.AddOnlyPublishedPaths) publishedRoutes = GetPublishedPaths(config);

                    Regex filterRegex = null;
                    if (!string.IsNullOrWhiteSpace(swagger.PathFilterRegexPattern)) filterRegex = new Regex(swagger.PathFilterRegexPattern);

                    foreach (var swaggerPath in swagger.Paths)
                    {
                        if (!Uri.TryCreate(new Uri(destination.Value.Address), swaggerPath, out Uri swaggerUrl)) throw new ArgumentException("Unable to combine specified url values");
                        ...
                        if (swagger.MetadataPath == swaggerPath) info = doc.Info;
                        ...
                        foreach (var path in doc.Paths)
                        {
                            ...
                            if (publishedRoutes != null)
                            {
                                ...
                                if (!publishedRoutes.ContainsKey(pathKey)) continue;
                                ...
                                foreach (var operationKey in operationKeys)
                                    if (!operations.Contains(operationKey)) path.Value.Operations.Remove(operationKey);
                            }
    ...
    private static Dictionary<string, IEnumerable<string>> GetPublishedPaths(
        ReverseProxyDocumentFilterConfig config)
    {
        var validRoutes = new Dictionary<string, IEnumerable<string>>();
        foreach (var route in config.Routes)
        {
            if (route.Value?.Match.Path == null) continue;
        ...
        KeyValuePair<string, IOpenApiPathItem> path, string clusterKey)
    {
        ...
        foreach (var operationKey in operationKeys)
        {
            ...
            foreach (var swaggerFactory in factories.Select(factory => factory as ISwaggerTransformFactory))
                foreach (var transform in transforms) swaggerFactory?.Build(operation, transform);
        }
    }
}